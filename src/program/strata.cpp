...
void Strata::check_rule_dependencies(size_t stratum_index, rule::Rule *rule,
                                     std::vector<Stratum> *temp_stratum_vector) {
...
void Strata::follow_rule_dependencies(
    ...
    for (const auto &iterator : predicate_map) {
        ...
        std::tie(predicate, formula_vector) = iterator;
        auto search = head_predicate_map.find(predicate);
        auto found = search != head_predicate_map.end();
        if (found) { // predicate is the head of another rule
            for (rule::Rule *head_predicate_rule : head_predicate_map[predicate]) {
...
bool Strata::evaluate(uint64_t time_point, uint64_t tuple_counter,
    ...
    for (auto stratum :stratum_vector) {
        result = result || stratum.evaluate(time_point, tuple_counter, new_facts);
    }
    return result;
}
