...
void RestrictedFilter::init(
    ...
    init_atom_head_var_index(head_atoms);
...
std::vector<std::string const &> RestrictedFilter::extract_substitution(
...
bool RestrictedFilter::is_substitution_match(
    std::vector<std::string const &> const &left,
    std::vector<std::string const &> const &right, ) const {
...
bool RestrictedFilter::has_match_current_timepoint(
    std::vector<std::string const &> const &initial_substitution,
...
bool RestrictedFilter::is_full_match(
    std::vector<std::string const &> const &substitution) const {
...
bool RestrictedFilter::is_fact_match(
    std::vector<std::string const &> const &substitution,
    std::shared_ptr<util::Grounding> const &fact, size_t head_atom_index) {
    ...
    for (size_t atom_var_index = 0; atom_var_index < atom_var_count;
         atom_var_index++) {
        auto head_var_index = var_index_atom_to_head(atom_var_index);
        if (head_var_index >= 0) {
...
std::vector<std::string const &> RestrictedFilter::fact_extend(
    std::vector<std::string const &> const &substitution,
    std::shared_ptr<util::Grounding> const &fact,
    size_t head_atom_index) const {
    std::vector<std::string const &> result = substitution;
    auto const &var_index_atom_to_head =
        head_atoms_var_index_atom_to_head.at(head_atom_index);
    auto atom_var_count = var_index_atom_to_head.size();
    for (size_t atom_var_index = 0; atom_var_index < atom_var_count;
...
std::vector<std::string const &> RestrictedFilter::event_extend(
    std::vector<std::string const &> const &left,
    std::vector<std::string const &> const &right) const {
    std::vector<std::string const &> result = left;
...
bool RestrictedFilter::search_database(
    ...
    std::vector<std::vector<std::string const &>> substitution_list;
    if (has_event_variables) {
    ...
    for (auto const &substitution : substitution_list) {
        bool match_found = find_database_match(substitution, 0);
        if (match_found) {
            return;
        }
    }
...
std::vector<std::vector<std::string const &>> RestrictedFilter::match_events(
    std::vector<std::string const &> const &initial_substitution,
    size_t head_atom_index) const {
    std::vector<std::vector<std::string const &>> result;
...
std::vector<std::vector<std::string const &>> RestrictedFilter::match_database(
    std::vector<std::string const &> const &initial_substitution,
    size_t head_atom_index) const {
    std::vector<std::vector<std::string const &>> result;
    ...
    if (next_head_atom < head_atom_count) {
        for (auto const &substitution : substitution_list) {
            bool found_extension =
                find_database_match(substitution, next_head_atom);
            if (found_extension) {
                return true;
            }
        }
