...
std::unique_ptr<formula::Formula> TimeWindow::clone() const {
    ...
    return std::make_unique<formula::TimeWindow>(
        past_size, future_size, step_size, std::move(child_clone));
}
...
std::vector<std::string> TimeWindow::get_math_arguments() const {
    std::vector<std::string> result;
    return result;
}

MathOperator TimeWindow::get_math_operator() const {
