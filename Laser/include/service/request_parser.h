...
class RequestParser {
  private:
    ...
    std::vector<laser::formula::Formula *> parse_atom_vector();
    std::stringstream input;
    std::unique_ptr<Request> parse_query();

    std::vector<size_t>
    compute_known_index_vector(formula::Formula *query_formula,
                               std::set<std::string> const &unknown_variables) const;

    util::Grounding
    compute_trigger(formula::Formula *query_formula) const;
